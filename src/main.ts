...
import { CapacitorUpdater, BundleInfo } from '@capgo/capacitor-updater'; // ⬅ Capgo
...
// ---------- Versiecheck + auto web-bundle update (Capgo) ----------
async function checkForUpdates() {
  try {
    ...
    // (optioneel) native minimum afdwingen
    const appInfo = await App.getInfo();
    const nativeOk = !manifest.minNativeVersion || cmpSemver(appInfo.version, manifest.minNativeVersion) >= 0;
    if (!nativeOk) {
      const go = confirm(`Nieuwe app-versie vereist (min. ${manifest.minNativeVersion}). Nu updaten?`);
      if (go && manifest.apkUrl) await Browser.open({ url: manifest.apkUrl });
      return;
    }

    // Markeer huidige bundle als ok (rollback-bescherming)
    try { await CapacitorUpdater.notifyAppReady(); } catch {}

    // Als manifest een nieuwere web-bundle heeft → download & set
    if (manifest.bundleVersion && manifest.bundleUrl) {
      // Huidige versie (optioneel: je kunt ook direct downloaden)
      const current = await CapacitorUpdater.current().catch(() => null);
      if (current?.version !== String(manifest.bundleVersion)) {
        ...
        await CapacitorUpdater.set(version); // activeert & herlaadt app automatisch
      }
    }
  } catch (e:any) {
...
async function startWatcher(){
  ...
  const opts: any = {
    requestPermissions:false, stale:false,
    backgroundTitle:'Dispatch tracking', backgroundMessage:'Live locatie actief',
    distanceFilter:0, stopOnTerminate:false, startOnBoot:true,
    interval:15000, fastestInterval:5000
  };
...
async function start(){
  await checkForUpdates();           // ⬅ auto-update check (Capgo)
  ...
  async function refresh(){
    try{
      ...
      j.rows.forEach((row:any)=>{
        const key=row.team; const lat=parseFloat(row.lat), lon=parseFloat(row.lon);
